...
// SetPeers updates this server's known peer IDs (excluding self) and informs
// the underlying ConsensusModule (if running). This is used by the test
// harness to support dynamic membership.
func (s *Server) SetPeers(peerIds []int) {
	s.mu.Lock()
	s.peerIds = peerIds
	cm := s.cm
	s.mu.Unlock()
